...
canvasStomp.connect({}, function () {
    ...
    canvasStomp.subscribe("/sub/meta/caferoom/canvas/" + canvasMetaIdx, function (message) {
        ...
        if ( canvasType == "reErr" ) {
            ...
            if ( canvasWriter == canvasNickname ) {
                ...
                if ( writer === metaNickname ) {
                    canvasStomp.send('/pub/meta/caferoom/canvas/enter', {}, JSON.stringify({type: "enter", metaIdx: canvasMetaIdx, writer: canvasNickname, character: canvasProfileImage, x : cw / 2, y : (ch / 6) * 5}));
                }
            }
        }
    ...
    // 재입장(새로고침)일 경우 - 입장 체크값이 존재한다.
        ...
        //    JSON.stringify({json형식}) - JavaScript 값이나 객체를 JSON 문자열로 변환한다.
        //    여기서 전송한 메시지를 StompChatController에 @MessageMapping이 DTO를 통해 받는다.
        canvasStomp.send('/pub/meta/caferoom/canvas/reenter', {}, JSON.stringify({type: "reEnter", metaIdx: canvasMetaIdx, writer: canvasNickname, character: canvasProfileImage, x : cw / 2, y : (ch / 6) * 5}));
    }
});
/////////////////////////////////////////////////// 캔버스 구역 ///////////////////////////////////////////////////
// 캔버스 메소드
...
function keyDown() {
    ...
    switch(keycode) {
        ...
        case 37:
            ...
            // JSON.stringify({json형식}) - JavaScript 값이나 객체를 JSON 문자열로 변환한다.
            // 여기서 전송한 메시지를 StompChatController에 @MessageMapping이 DTO를 통해 받는다.
            canvasStomp.send('/pub/meta/caferoom/canvas/move', {}, JSON.stringify({type: "left", metaIdx: canvasMetaIdx, writer: canvasNickname, character: canvasProfileImage, canvasLeft: canvasLeft, canvasTop: canvasTop, canvasRight: canvasRight, canvasBottom: canvasBottom}));
            break;
        // 위로 이동
        case 38:
            ...
            // JSON.stringify({json형식}) - JavaScript 값이나 객체를 JSON 문자열로 변환한다.
            // 여기서 전송한 메시지를 StompChatController에 @MessageMapping이 DTO를 통해 받는다.
            canvasStomp.send('/pub/meta/caferoom/canvas/move', {}, JSON.stringify({type: "top", metaIdx: canvasMetaIdx, writer: canvasNickname, character: canvasProfileImage, canvasLeft: canvasLeft, canvasTop: canvasTop, canvasRight: canvasRight, canvasBottom: canvasBottom}));
            break;
        // 오른쪽으로 이동
        case 39:
            ...
            // JSON.stringify({json형식}) - JavaScript 값이나 객체를 JSON 문자열로 변환한다.
            // 여기서 전송한 메시지를 StompChatController에 @MessageMapping이 DTO를 통해 받는다.
            canvasStomp.send('/pub/meta/caferoom/canvas/move', {}, JSON.stringify({type: "right", metaIdx: canvasMetaIdx, writer: canvasNickname, character: canvasProfileImage, canvasLeft: canvasLeft, canvasTop: canvasTop, canvasRight: canvasRight, canvasBottom: canvasBottom}));
            break;
        // 아래로 이동
        case 40:
            ...
            // JSON.stringify({json형식}) - JavaScript 값이나 객체를 JSON 문자열로 변환한다.
            // 여기서 전송한 메시지를 StompChatController에 @MessageMapping이 DTO를 통해 받는다.
            canvasStomp.send('/pub/meta/caferoom/canvas/move', {}, JSON.stringify({type: "bottom", metaIdx: canvasMetaIdx, writer: canvasNickname, character: canvasProfileImage, canvasLeft: canvasLeft, canvasTop: canvasTop, canvasRight: canvasRight, canvasBottom: canvasBottom}));
            break;
    }
}
